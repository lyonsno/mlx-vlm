...
class TestBatchGenerate:
    ...
    def test_generate_batch_keeps_ordinary_3d_prompt_kwargs_batch_first(
        self, mock_model, mock_processor
    ):
        class _EmbeddingOutput:
            def __init__(self, inputs_embeds, ordinary_3d):
                self.inputs_embeds = inputs_embeds
                self.ordinary_3d = ordinary_3d

            def to_dict(self):
                return {
                    "inputs_embeds": self.inputs_embeds,
                    "ordinary_3d": self.ordinary_3d,
                }
        ...
        ordinary_3d = inputs_embeds + 100
        embedding_output = _EmbeddingOutput(inputs_embeds, ordinary_3d)
        ...
        def fake_insert(
            ...
            for i, kw in enumerate(prompt_kwargs):
                assert kw["inputs_embeds"].shape == (1, seq_len, hidden_size)
                assert kw["ordinary_3d"].shape == (1, seq_len, hidden_size)
                assert kw["inputs_embeds"].tolist() == inputs_embeds[i : i + 1].tolist()
                assert kw["ordinary_3d"].tolist() == ordinary_3d[i : i + 1].tolist()
            raise _StopInsert
